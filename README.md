...
> **一句话**：车载语音助手自动化评估系统 → 解决数百条多轮对话 case 无法人工逐条评测、难以汇总分析的问题 → Dify Workflow + Python（asyncio/aiohttp）+ LLM + matplotlib → `pip install -r requirements.txt`，配置 `.env` 后依次运行 `python src/run_eval.py`、`python src/batch_summary.py`、`python src/analyze_reports.py`。

这是一个专为车载语音助手（In-Car Voice Assistant）设计的自动化批量评估、汇总与语义分析系统。项目结合了 **Dify Workflow** 自动化测试流程与大语言模型（如 Qwen 3.5）的深度分析能力，能够对数百个测试用例（Cases）进行批量并发测试，自动生成可视化图表及多维度的评估报告。
...
```mermaid
flowchart TD
    A["① 准备测试数据<br/>多轮对话 query + 车辆上下文 case"] --> B["② 批量调用 Dify 工作流<br/>异步并发跑完所有测试 case"]
    B --> C["③ 生成单 case 评估报告<br/>每个 case 一份 Markdown"]
    C --> D["④ 报告分批合并<br/>按批汇总并自动生成目录"]
    D --> E["⑤ 汇总分析与可视化<br/>解析指标、绘制图表"]
    E --> F["⑥ LLM 深度语义分析<br/>错误模式归纳与改进建议"]
    F --> G["⑦ 输出最终成果<br/>评估报告 + 可视化图表"]
```
...
准备测试数据（多轮对话 + 车辆上下文）
        ↓
批量调用 Dify 工作流，异步并发跑完所有 case
        ↓
为每个 case 生成 Markdown 评估报告
        ↓
将报告按批合并，并自动生成目录导航
        ↓
解析报告、统计指标，绘制得分分布与混淆矩阵等图表
        ↓
调用 LLM 做语义分析与错误模式归纳
        ↓
输出最终评估报告与可视化图表
